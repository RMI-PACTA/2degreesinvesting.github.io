---
title: "r2dii.data 0.1.6, r2dii.match 0.0.7 and r2dii.analysis 0.1.2 are now on CRAN"
description: |
...
r2dii.data 0.1.6, r2dii.match 0.0.7 and r2dii.analysis 0.1.2 are now on CRAN. These releases fix a number of bugs. All changes are listed in the changelog at each packages website (e.g. [here](https://2degreesinvesting.github.io/r2dii.analysis/news/index.html) for r2dii.analysis); this post highlights the major bugs fixes and feature additions.
...
The only user-facing change to r2dii.match was the addition of the `...` argument which allows further arguments to be passed directly the the `stringdist::stringsim()` function. This feature allows advanced users to have more flexibility over the particular matching algorithm implemented in the `match_name()` function. 

For most users, the default values are likely still sufficient: 
...
however, the `q` parameter of the `qgram` method, for example, can now be tweaked directly:
...
Let's calculate some more demo data frames, that we can use to help explain the rest of the bugfixes:
...
`target_market_share` outputs a data frame with the value `technology_share`. This value is expected to always sum to `1`, even when aggregated across appropriate groupings (e.g. by `sector`, `region`, `metric`). Before, this was true only for results at the level of companies but not portfolios. Now, this is fixed and `technology_share` sums `1` at both levels (at least to 10 significant digits; further digits may be off due to rounding errors):
